...
pcklibs <- c("dplyr", 'RJSONIO', 'TTR')#"arm", xts httr",RCurl, caret, 
lapply(pcklibs, require, character.only=TRUE)



ss.BTCUSDT <- fileSnapshot(path="/ubuntu/home/BTCUSDT", file.info = T)
ss.ETHUSDT <- fileSnapshot(path="/ubuntu/home/ETHUSDT", file.info = T)
ss.BNBUSDT <- fileSnapshot(path="/ubuntu/home/BNBUSDT", file.info = T)

dx.BTCUSDT <- changedFiles(ss.BTCUSDT, path=path = "/ubuntu/home/BTCUSDT")
dx.ETHUSDT <- changedFiles(ss.ETHUSDT, path=path = "/ubuntu/home/ETHUSDT")
dx.BNBUSDT <- changedFiles(ss.BNBUSDT, path=path = "/ubuntu/home/BNBUSDT")
...
for (Cpair in pair.vecotor){
  for (changes in change.list){ #or should 'changes' be 'i'?

#NEW data
#####################
print('NEW data incoming!')
KlinesResp <-  rjson::fromJSON(file=paste0(Cpair, "/klines@symbol=", Cpair, "&interval=5m.", i)) #need to udpate 'i'
...
BookResp <-  rjson::fromJSON(file=paste0(Cpair, "/bookTicker@symbol=", Cpair, ".", i)) #need to udpate 'i'    #"NANO/bookTicker@symbol=NANOBTC."
...
DepthResp <-  rjson::fromJSON(file=paste0(Cpair, "/depth@symbol=", Cpair, ".", i)) #need to udpate 'i'    #"NANO/bookTicker@symbol=NANOBTC."
...
KlinesRespBTC <- rjson::fromJSON(file=paste0("BTCUSDT/klines@symbol=BTCUSDT&interval=5m.", i))
...
BookRespBTC <- rjson::fromJSON(file=paste0("BTCUSDT/bookTicker@symbol=BTCUSDT.", i))
...
#SeedData <- rbind(SeedData, BinanceObs)
#BinanceData.f <- SeedData
