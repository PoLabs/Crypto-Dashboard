...
pair.vector <- c('BCCBTC', 'LTCBTC', 'XRPBTC', 'XMRBTC', 'VENBTC', 'OMGBTC', 'ICXBTC', 'ONTBTC', 'NANOBTC', 'XEMBTC')#c('BTCUSDT','ETHUSDT','BNBUSDT')

ss.new.BCCBTC <- fileSnapshot(path="/home/ubuntu/cryptoAPI/BCCBTC/raw", file.info = T) #"/ho$
ss.new.LTCBTC <- fileSnapshot(path="/home/ubuntu/cryptoAPI/LTCBTC/raw", file.info = T) #"/ho$
ss.new.XRPBTC <- fileSnapshot(path="/home/ubuntu/cryptoAPI/XRPBTC/raw", file.info = T)
ss.new.XMRBTC <- fileSnapshot(path="/home/ubuntu/cryptoAPI/XMRBTC/raw", file.info = T)
ss.new.VENBTC <- fileSnapshot(path="/home/ubuntu/cryptoAPI/VENBTC/raw", file.info = T)
ss.new.OMGBTC <- fileSnapshot(path="/home/ubuntu/cryptoAPI/OMGBTC/raw", file.info = T)
ss.new.ICXBTC <- fileSnapshot(path="/home/ubuntu/cryptoAPI/ICXBTC/raw", file.info = T)
ss.new.ONTBTC <- fileSnapshot(path="/home/ubuntu/cryptoAPI/ONTBTC/raw", file.info = T)
ss.new.NANOBTC <- fileSnapshot(path="/home/ubuntu/cryptoAPI/NANOBTC/raw", file.info = T)
ss.new.XEMBTC <- fileSnapshot(path="/home/ubuntu/cryptoAPI/XEMBTC/raw", file.info = T)

ss.new.list <- list(ss.new.BCCBTC, ss.new.LTCBTC, ss.new.XRPBTC, ss.new.XMRBTC, ss.new.VENBTC, ss.new.OMGBTC, ss.new.ICXBTC, ss.new.ONTBTC, ss.new.NANOBTC, ss.new.XEMBTC)
...
for(Cpair in pair.vector){
  ...
  runnum <- 1
  allNames <- as.character(row.names(ss.new.list[[Cpair]]$info))# pul l all timess
  ...
  # for(time in (length(uniqTimes)-2250):length(uniqTimes)){
    ...
    for(x in (which(uniqTimes %in% c(SeedData[nrow(SeedData)-10,28], 'holder'))):length(uniqTimes)){   #TxN matches last Seed time to nrow TxN.df
      ...
      if(length(position.vec) > 2){
        for(i in length(position.vec)){
          ...
          }, error = function(e) print(paste('error in ', uniqTimes[x]))  # falseflag <- 1#{         
          )
        }
        tryCatch({
