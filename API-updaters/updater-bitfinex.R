...
for(Cpair in pair.vector){
  ...
  if(SeedData[nrow(SeedData),20] != uniqTimes[length(uniqTimes)]){ #TxN.df[nrow(TxN.df),2]){
    for(x in (which(uniqTimes %in% c(SeedData[nrow(SeedData),20], 'holder'))-1):length(uniqTimes)){   #TxN matches last Seed time to nrow TxN.df
