...
pair.vector <- c('BTCUSDT')#, 'TRXBTC', 'ADABTC', 'EOSBTC', 'IOTABTC', 'NEOBTC', 'XLMBTC', 'ETHUSDT', 'BNBUSDT', 'BNBBTC', 'ETHBTC')
...
for(pair in pair.vector){
  ...
  for (i in 2:6){
    ...
    #Ensemble
    Ensemble_control_A1 <- trainControl(method = "repeatedcv",  number = 15,  repeats = 2,  verboseIter = TRUE,  returnData = FALSE, trim=TRUE, returnResamp = "all", 
