...
DF.interest <- as.data.frame(c(1,2,3))

for (Cpair in pair.vector){
  
  Cpath <- paste0('/root/cryptoAPI/NLP/', Cpair, '/CSV')
  ...
  # }
  tracker.c.location <- paste0(Cpair, '/sync/', Cpair, '-tracker.csv')
  tracker.current <- as.data.frame(fread(file=tracker.c.location))
  
  print(paste0('reddit num - nrow tracker:', Reddit.num - nrow(tracker.current)))
  
  if(Reddit.num > nrow(tracker.current)){

    for(n in 1:(Reddit.num-nrow(tracker.current))){
      print(n)
      
      
      #loop over: #files -  nrow tracker
      
      #1. read in reddit comment csv
      read.location <- paste0(Cpair, '/CSV/', Cpair, '-nlp-', (nrow(tracker.current)+n), '.csv')
      print(read.location)
      ...
      track.location <- paste0(Cpair, '/sync/', Cpair,'-tracker.csv')
